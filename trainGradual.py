...
def train(epoch, step):
    ...
    for batch_idx, (input1, input2, input3, label1, label2, _, cam1, cam2) in enumerate(trainloader):
        input1 = input1[label1 != -1]
        input2 = input2[label2 != -1]
        ...
        if batch_idx % 50 == 0 or batch_idx == len(trainloader) - 1:
            print('Epoch: [{}-{}][{}/{}] '
                  ...
                  'Accu: {:.2f}'.format(
                epoch, step, batch_idx, len(trainloader), current_lr,
...
for step in range(0, N):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        trainset.transform = updateTransform(0.2 + ((step+1) / N) * (epoch / end_epoch) * 0.4 )
        print(epoch)
